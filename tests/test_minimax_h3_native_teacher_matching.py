...
from musubi_tuner.minimax_h3_native.model import MiniMaxH3Config, MiniMaxH3Model
from musubi_tuner.minimax_h3_native.packing import H3ReferenceGeometry, H3VideoGeometry, build_h3_layout
...
from musubi_tuner.minimax_h3_native_train_network import (
    H3SamplingResources,
...
def test_h3_trainer_passes_backward_mode_to_loader_and_excludes_int8_linears_from_compile(monkeypatch):
    import musubi_tuner.minimax_h3_native_train_network as train
...
def test_compute_loss_is_video_mean_plus_weighted_audio_mean_mse():
    ...
    assert metrics == {
        "loss/video": pytest.approx(10.0),
        "loss/audio": pytest.approx(2.0),
        "loss/video_weight": pytest.approx(1.0),
    }
...
def test_h3_training_metadata_records_task_scheduler_and_target_policy():
    ...
    assert metadata == {
        ...
        "ss_minimax_h3_audio_only": False,
...
def test_h3_lora_gets_gradients_with_checkpointing_and_block_swap(monkeypatch):
    ...
    monkeypatch.setattr(
        "musubi_tuner.minimax_h3_native.model.create_offloader",
...
def _uncond_cache(tmp_path, *, rows: int = 2, width: int = 12, value: float = 0.0) -> str:
    from musubi_tuner.minimax_h3_native.text_encoder import save_h3_uncond_cache
...
def test_h3_uncond_cache_round_trips_and_rejects_foreign_formats(tmp_path):
    ...
    from musubi_tuner.minimax_h3_native.text_encoder import load_h3_uncond_cache
