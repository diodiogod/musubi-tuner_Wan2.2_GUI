...
const HELP = {
  ...
  recache_latents: "Rebuild the cached image information before training. Enable this for the first run or after changing images, resolution, or VAE. If you add images to the dataset, review and enable this; the current GUI may reprocess existing images too because incremental cache updates are not implemented yet.",
  recache_text: "Rebuild the cached caption information before training. Enable this for the first run or after changing captions or the text encoder. If you add images or captions, review and enable this; the current GUI may reprocess existing entries too because incremental cache updates are not implemented yet.",
  ...
  minimax_h3_training_preview_mode: "Choose whether scheduled MiniMax training samples use the recommended five-frame video or a lower-memory one-frame diagnostic. This does not change each card's standalone Preview setting.",
  minimax_h3_quality_protection_method: "Choose how MiniMax H3 keeps its original generation quality while the new LoRA learns. Dynamic Sigma is the proven default. The Ostris v1 assistant modes remain experimental.",
  minimax_h3_quality_protection_preset: "Applies a sensible combination, then leaves every control editable. Proven Quality is the validated Dynamic Sigma configuration; the other presets are experimental.",
  minimax_h3_training_assistant_enabled: "Keeps Ostris's frozen v1 helper active during training steps. It is disabled for previews and never saved into your LoRA.",
  minimax_h3_dynamic_sigma_enabled: "Enables the proven empty-prompt quality target. It can run every step or periodically alongside the assistant.",
  ...
  minimax_h3_foundation_lora_enabled: "Keeps an existing H3 LoRA frozen and active while a separate new LoRA trains on top. This protects the existing file from further training. The new LoRA may depend on the foundation LoRA during generation.",
  minimax_h3_foundation_lora: "The already-trained MiniMax H3 LoRA whose behavior should form the starting point. It stays active during training and previews but is never copied into the new output.",
  minimax_h3_foundation_lora_multiplier: "Fixed strength for the foundation throughout training and previews. Start with 1.0 or the strength at which the existing LoRA already works reliably.",
  krea2_projector_diff: "A small Krea-specific correction patch such as krea2filterbypass3.safetensors. It directly adjusts only projector weights before training. It is not a normal LoRA and does not continue your trained LoRA. Leave it blank unless a model recipe specifically calls for it.",
  krea2_projector_diff_strength: "How strongly to apply the specialized projector correction. Use the value recommended by the patch author; 1.0 means its original strength. This is not a LoRA strength control.",
  ...
  minimax_h3_base_preservation_enabled: "Adds an independent drift penalty on its scheduled steps. This is optional and is not the same mechanism as Dynamic Sigma.",
  minimax_h3_base_preservation_reference: "Base + assistant measures drift from the helper-assisted baseline. Base only temporarily disables both the user's LoRA and the helper for the reference pass.",
  ...
  timestep_sampling: "Compact MiniMax defaults to krea2_shift because that is our proven recipe. h3_shifted_uniform is an experimental full-schedule alternative inspired by Fizgig's finding; it preserves substantially more high-noise pose/composition practice.",
...
const LONG_HELP_COPY = {
  ...
  timestep_sampling: "This controls which noise levels the model practices during training. Each mode normally selects an appropriate value.\n\nCompact MiniMax H3 keeps krea2_shift as the proven default. The experimental h3_shifted_uniform option follows H3's shift-12 inference schedule from a uniform base draw, retaining much more practice at the high-noise end where pose and composition form. It is exposed for community testing after Fizgig reported that middle-concentrated training could distort ordinary 20-step inference. Changing it does not require rebuilding either cache.",
  ...
  krea2_projector_diff: "This field is for a specialized Krea projector correction such as krea2filterbypass3.safetensors. The file contains direct differences for a small projector portion of the model; it is not an ordinary LoRA and cannot be used to continue a character, style, or concept LoRA.\n\nThe correction is applied to the frozen RAW model before training and to optional Turbo previews so both paths use the same projector behavior. Leave it blank unless the patch's recipe explicitly requires it, and use that recipe's recommended strength.",
  ...
  minimax_h3_training_preview_mode: "Five-frame video is recommended because it represents MiniMax H3 video quality better and saves a short MP4. It has no training gradients, but it is slower and can temporarily require more VRAM.\n\nOne frame is available as a faster, lower-memory diagnostic and may look blurrier than real video output. This controls only scheduled in-training samples; each prompt card keeps its own frame count for the standalone Preview button.",
  minimax_h3_foundation_lora_enabled: "Enable this when you already have a proven MiniMax H3 LoRA and want to teach a separate, narrower refinement without modifying the proven file. The foundation stays frozen and active while a fresh output LoRA learns on top.\n\nThis is different from Continue from LoRA, which loads prior weights into the adapter being trained. The refinement output does not contain the foundation, so generation normally needs both LoRAs at the strengths used during training.",
  minimax_h3_foundation_lora: "Select an ordinary MiniMax H3 LoRA produced by Musubi or this GUI. It is attached as a frozen runtime adapter, which works with the pruned ConvRot INT8 base without rewriting its quantized weights.\n\nThe foundation remains enabled for scheduled previews so those samples represent the same model combination used by training. The selected file is read only and is never included in the newly saved LoRA.",
  minimax_h3_quality_protection_method: "Dynamic Sigma is this project's tested method. It makes an extra empty-prompt prediction every step and has already survived very long runs without destroying H3 quality. It is slower and needs the Caption/Text Cache rebuilt once.\n\nOstris Assistant loads Ostris's published v1 helper LoRA as a frozen training aid. It adds about 155 MB of weights, needs no unconditional text cache, and is expected to be faster because it avoids the extra pass. The helper is disabled for samples and is not saved into your LoRA.\n\nAssistant + Base Preservation keeps that helper active and occasionally compares the trainable LoRA against the frozen base. With the default cadence of 10, the expensive comparison happens on only one in ten steps. This hybrid is promising but less validated than Dynamic Sigma.\n\nOff is useful only for controlled comparisons or short runs where quality drift is acceptable.",
  minimax_h3_quality_protection_preset: "Presets set all three independent quality layers but do not lock them. Proven Quality is the validated Dynamic Sigma 4.0 configuration on every step. Balanced combines the helper with Dynamic Sigma every 10 steps; Strong uses every 4; Maximum also turns on the separate drift check.\n\nChanging any individual control marks the recipe Custom. This is expected and does not disable anything.",
  minimax_h3_training_assistant_enabled: "This attaches Ostris's published v1 helper LoRA to the frozen H3 transformer while your own LoRA trains. The helper stays frozen, is absent from the optimizer, is temporarily disabled for previews, and is never written into your output.\n\nIt can be combined freely with periodic or every-step Dynamic Sigma and with the optional drift check.",
  minimax_h3_dynamic_sigma_enabled: "Dynamic Sigma is the quality mechanism already validated in long training. On its scheduled steps it performs an empty-prompt prediction without gradients and builds the protected target using the selected strength and schedule.\n\nIt works with or without the Ostris assistant. Enabling it requires one Caption/Text Cache rebuild so each item contains the empty-prompt state.",
  minimax_h3_dynamic_sigma_every_n_steps: "1 applies the proven correction every training step and adds one no-gradient DiT pass per step. 4 applies it on every fourth step, averaging 25% of that extra cost. 10 applies it on every tenth step, averaging 10%.\n\nPeriodic values are experimental. They are primarily intended for combining the always-active Ostris helper with the established Dynamic Sigma target.",
  minimax_h3_training_assistant: "The default points to ostris/minimax_h3_training_adapter on Hugging Face and downloads the v1 safetensors file automatically on first use. Later runs reuse the local Hugging Face cache. A local .safetensors path is also accepted.\n\nThe helper is attached live and frozen: the optimizer cannot change it, training previews temporarily turn it off, and it is not merged into the user's final LoRA. This integration references Ostris's published adapter format while retaining Musubi's normal LoRA output.",
  minimax_h3_base_preservation_loss_weight: "On comparison steps, the trainer temporarily turns off only your trainable LoRA, records the frozen base-plus-helper prediction without gradients, then penalizes large differences from the normal trainable prediction.\n\n0.05 is a conservative experimental starting point. Raising it may preserve the base more strongly but can prevent the desired concept from learning. This is not DOP and does not use regularization images or class captions.",
  minimax_h3_base_preservation_enabled: "This optional drift check is a third mechanism. It does not replace Dynamic Sigma. On its scheduled steps it makes a frozen reference prediction with the user's LoRA disabled, then adds an MSE penalty to discourage excessive drift.\n\nLeave it off for the first assistant-plus-periodic-Dynamic test. Enable it only when you deliberately want another preservation signal.",
  minimax_h3_base_preservation_reference: "Base + assistant disables only the user's trainable LoRA for the reference prediction, keeping Ostris's helper in both sides of the comparison. Base only also disables the helper during that reference.\n\nBase + assistant is the more internally consistent starting point when the helper is enabled. Base only is a stronger and less validated experiment.",
  minimax_h3_base_preservation_every_n_steps: "This controls speed versus protection. 10 performs the base comparison once every ten optimizer steps; 5 does it twice as often; 1 performs it every step and loses most of the helper's speed advantage.\n\nStart at 10. Use a smaller number only if previews show drift, and a larger number only after the v1 hybrid has been validated for your kind of LoRA.",
...
function fieldControl(field, {wide = false} = {}) {
  ...
  const commit = () => {
    ...
    if(field.key==="minimax_h3_dynamic_sigma_enabled"&&input.checked&&!state.settings.recache_text){
        state.settings.recache_text=true;
        toast("Caption/Text Cache rebuilding was enabled once for Dynamic Sigma protection.");
    }
    if(field.key==="minimax_h3_teacher_matching"&&input.checked){
      state.settings.minimax_h3_multimodal_task="t2va";
    ...
    if(field.key.startsWith("minimax_h3_")&&[
      ...
      "minimax_h3_guidance_distillation_scale","minimax_h3_guidance_distillation_schedule",
      "minimax_h3_base_preservation_enabled","minimax_h3_base_preservation_loss_weight",
      "minimax_h3_base_preservation_every_n_steps","minimax_h3_base_preservation_reference"
    ].includes(field.key)){
      state.settings.minimax_h3_quality_protection_preset="Custom";
...
function markUnavailable(host, message) {
  const notice=document.createElement("div");notice.className="issue warning";notice.textContent=message;host.prepend(notice);
  host.querySelectorAll("input, select, textarea, button:not(.help)").forEach(control=>{
...
function renderGuided() {
  ...
  if(mode==="MiniMax H3 (Experimental)"&&h3Multimodal)markUnavailable($("#dop-fields"),"Not available in native video/audio training yet. The setting is shown so the limitation is visible, but it is not sent to the native trainer.");
  ...
  if(h3Multimodal){
    ...
    if(state.settings.minimax_h3_teacher_matching)visibleH3Protection.push("minimax_h3_teacher_conditions","minimax_h3_teacher_condition_sigma_max","minimax_h3_teacher_loss_dc_weight","minimax_h3_teacher_loss_mag_weight","minimax_h3_teacher_preservation_weight","minimax_h3_timestep_focus_min","minimax_h3_timestep_focus_max","minimax_h3_timestep_focus_prob");
      ...
      visibleH3Protection.push("minimax_h3_dynamic_sigma_enabled","minimax_h3_guidance_distillation_scale","minimax_h3_guidance_distillation_sigma_min");
    }
  ...
  h3Foundation.hidden=mode!=="MiniMax H3 (Experimental)";
  const foundationKeys=["minimax_h3_foundation_lora_enabled"];
  if(state.settings.minimax_h3_foundation_lora_enabled)foundationKeys.push("minimax_h3_foundation_lora","minimax_h3_foundation_lora_multiplier");
  appendFields($("#minimax-foundation-fields"),foundationKeys);
  if(h3Multimodal){
    const unavailableKeys=["minimax_h3_base_preservation_enabled","minimax_h3_base_preservation_loss_weight","minimax_h3_base_preservation_every_n_steps","minimax_h3_base_preservation_reference"];
    unavailableKeys.forEach(key=>{
      const field=$("#minimax-guidance-fields").querySelector(`[data-key="${key}"]`);
      if(field)field.querySelectorAll("input, select, textarea, button:not(.help)").forEach(control=>{control.disabled=true;control.title="Compact still-image trainer only; native video integration is not implemented yet."});
    });
    const notice=document.createElement("div");notice.className="issue warning";notice.textContent="The Ostris Assistant is available for native video/audio training and may be combined with Dynamic Sigma. It is an alternative to Teacher Matching. Drift/Base Preservation remains shown but disabled: that periodic comparison is our separate downstream experiment and is not ported yet.";$("#minimax-guidance-fields").append(notice);
  }
...
function selectMode(mode) {
  ...
  if(mode === "MiniMax H3 (Experimental)"){
    Object.assign(state.settings, {
      ...
      minimax_h3_training_assistant_enabled:state.settings.minimax_h3_training_assistant_enabled??false,
      minimax_h3_dynamic_sigma_enabled:state.settings.minimax_h3_dynamic_sigma_enabled??true,
...
function renderReview() {
  ...
  if(state.settings.training_mode==="MiniMax H3 (Experimental)"){
    ...
    if(state.settings.minimax_h3_foundation_lora_enabled)rows.push([
      "Frozen foundation",
      `${String(state.settings.minimax_h3_foundation_lora||"Not selected").split(/[\\/]/).pop()} at ${state.settings.minimax_h3_foundation_lora_multiplier||"1.0"}×`,
    ]);
    rows.push(["Quality protection",native&&state.settings.minimax_h3_teacher_matching
      ? `Teacher matching · ${state.settings.minimax_h3_teacher_conditions||"ref"} · cutoff ${state.settings.minimax_h3_teacher_condition_sigma_max||"0.75"}`
      : state.settings.minimax_h3_dynamic_sigma_enabled?`Dynamic Sigma · minimum ${state.settings.minimax_h3_guidance_distillation_sigma_min||"0.15"}`:"Dynamic Sigma off"]);
  }
...
function applyH3QualityPreset(preset){
  const values={
    "Proven Quality":{assistant:false,dynamic:true,dynamicEvery:"1",base:false},
    "Experimental Balanced":{assistant:true,dynamic:true,dynamicEvery:"10",base:false},
    "Experimental Strong":{assistant:true,dynamic:true,dynamicEvery:"4",base:false},
    "Maximum Protection":{assistant:true,dynamic:true,dynamicEvery:"1",base:true}
  }[preset];
  if(!values)return false;
  Object.assign(state.settings,{
    minimax_h3_quality_protection_preset:preset,
    minimax_h3_training_assistant_enabled:values.assistant,
    minimax_h3_dynamic_sigma_enabled:values.dynamic,
  ...
  if(values.dynamic)state.settings.recache_text=true;
  renderGuided();sync();toast(`${preset} quality-protection settings applied.`);return true;
}
...
function renderPlanOverview(){
  ...
  const fiveFrameScheduled=h3&&["five_frame","Five-frame video (experimental)","Five-frame video (recommended)"].includes(state.settings.minimax_h3_training_preview_mode);
...
function renderPlanPromptEditor(){
  ...
  if(["Krea 2","MiniMax H3 (Experimental)"].includes(mode)){
    ...
    previewSource.querySelector(".guided-fields").append(
      objectField("Generate this card with","_preview_use_lora",promptPreviewUsesLora(prompt)?"lora":"base",value=>prompt._preview_use_lora=value==="lora",{type:"select",options:[{label:state.settings.minimax_h3_foundation_lora_enabled?"Foundation + current LoRA":"Current / selected LoRA",value:"lora"},{label:"Base model only",value:"base"}],help:state.settings.minimax_h3_foundation_lora_enabled?"LoRA mode loads the frozen foundation first and the current refinement second, matching training. Base model only disables both for a clean control.":"Choose LoRA to test what training learned, or Base to create a control image without the LoRA."}),
...
function clientTrainingSettingsSummary(){
  ...
  if(settings.minimax_h3_foundation_lora_enabled&&String(settings.minimax_h3_foundation_lora||"").trim())parts.push(`foundation=${String(settings.minimax_h3_foundation_lora).split(/[\\/]/).pop()}@${settings.minimax_h3_foundation_lora_multiplier||1}`);
...
function renderComparison(index) {
  ...
  const renderMode=()=>{
    ...
    const comparisonMedia=[...host.querySelectorAll(".compare-media")];comparisonMedia.forEach(item=>item.addEventListener(isVideo?"loadedmetadata":"load",()=>requestAnimationFrame(fitComparisonHeight),{once:true}));const fitTarget=compareState.mode==="side"?host.querySelector(".compare-image"):host.querySelector(".wipe-stage");host._compareResizeObserver?.disconnect();if(window.ResizeObserver&&fitTarget){host._compareResizeObserver=new ResizeObserver(()=>fitComparisonHeight());host._compareResizeObserver.observe(fitTarget)}requestAnimationFrame(fitComparisonHeight);
...
async function prepareJob(row,action){const completed=action==="recovery"&&row.querySelector(".job-status")?.textContent==="completed";if(!confirmWorkspaceReplacement(action==="recovery"?(completed?"Load this completed run for exact extension?":"Load this exact recovery?"):"Load this continuation?"))return;const p=await api(`/api/jobs/prepare-${action}`,{method:"POST",body:JSON.stringify({source:row.dataset.source,index:Number(row.dataset.index)})});discardWorkspaceDrafts();state.settings=p.settings;renderGuided();renderAllSettings();sync();const datasetLoaded=await loadDatasetForSettings();go("setup");setStep("review");const label=action==="recovery"?(completed?"Exact extension loaded. Increase Max Train Epochs before starting.":"Verified recovery loaded for review."):"Continuation loaded for review.";toast(datasetLoaded?label:`${label} Its saved Dataset TOML could not be loaded; choose a valid file before starting.`,datasetLoaded?"info":"error")}
