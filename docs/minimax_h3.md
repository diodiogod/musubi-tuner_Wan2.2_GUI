...
with the same filename stem (`clip.mp4` + `clip.wav`). **What Should This LoRA Learn?** provides
three explicit targets: joint video+audio, video-only, or experimental audio-only. Audio-only still
needs a synchronized target video for every audio clip because H3 is a joint video/audio model; it
is not a raw-audio dataset mode. It sets the video loss to zero and requires real embedded,
sidecar, or JSONL audio for every sample.

In the Modern Dataset page, open **MiniMax media builder** to preview audio detection, convert a
folder of still images plus audio into synchronized H3 clips, or visually author Ref2VA records.
The Classic Dataset Config Builder has a **MiniMax media** tab for the same still+audio conversion
and Ref2VA validation. Generated clips are padded or trimmed to the chosen valid H3 frame count
(124 by default), use 24 fps video and 32 kHz stereo audio, copy a matching caption sidecar, and
are never allowed to overwrite an existing clip. Ref2VA is the advanced route and requires the
JSONL format below so every reference has an explicit role and order.
...
```bash
python src/musubi_tuner/minimax_h3_native_cache_latents.py \
...
```bash
python src/musubi_tuner/minimax_h3_native_cache_text_encoder_outputs.py \
  --dataset_config /data/h3/dataset.toml \
  --task t2va \
...
```bash
accelerate launch --num_cpu_threads_per_process 1 --mixed_precision bf16 src/musubi_tuner/minimax_h3_native_train_network.py \
  --dataset_config /data/h3/dataset.toml \
  --task t2va \
...
The native trainer can also keep Ostris's published v1 training-assistant LoRA live and frozen with `--h3_training_assistant_enabled --h3_training_assistant PATH_OR_HF_FILE`. This follows AI Toolkit's native H3 design: the helper is active during optimization, disabled for training previews, and never saved into the user's LoRA. It can be combined with Dynamic Sigma, but is an alternative to Teacher Matching. The GUI still shows Drift/Base Preservation disabled for native video because that periodic comparison is a separate downstream experiment, not behavior supplied by Ostris's trainer.
...
In either GUI, select the native **Video + audio** workflow, choose **t2va**, and enable
**Use Upstream Teacher Matching** in Training Quality Protection. The GUI defaults to the
upstream identity starting recipe (`ref`, cutoff `0.75`, DC weight `0.3`, focus probability
`0.5`) and exposes every value. It automatically requests the required text-cache rebuild;
the `first,last` teacher also rebuilds latent caches with FL2VA endpoints. Teacher matching
and Dynamic Sigma are alternatives and are never emitted together.
...
- `--h3_teacher_loss_mag_weight` (default 1.0) weights the magnitude term relative to direction on conditioned teaching steps. Lower it to prioritize direction matching (0 = pure direction); preservation-anchor steps always retain the full magnitude term because it restores the base output norm.
...
```bash
python src/musubi_tuner/minimax_h3_native_generate_video.py \
  --task t2va \
  --dit /models/minimax_h3_fl2va_bf16.safetensors \
  --video_vae /models/minimax_h3_video_vae_fp16.safetensors \
  --audio_vae /models/minimax_h3_audio_vae_fp32.safetensors \
