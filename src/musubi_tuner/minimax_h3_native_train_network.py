...
from musubi_tuner.minimax_h3_native.audio_vae import load_audio_vae
from musubi_tuner.minimax_h3_native.generation_inputs import (
...
from musubi_tuner.minimax_h3_native.media import H3_AUDIO_SPEC, audio_latent_frames, video_latent_frames
from musubi_tuner.minimax_h3_native.model import load_h3_transformer
    ...
    if teacher_conditions == TEACHER_CONDITIONS_REF:
        ...
        if has_fl_teacher_text:
            raise ValueError(
                ...
                " re-run minimax_h3_native_cache_text_encoder_outputs.py --task t2va --teacher_conditions ref"
            )
        ...
        if has_ref_teacher_text:
            raise ValueError(
                ...
                " re-run minimax_h3_native_cache_text_encoder_outputs.py --task t2va --teacher_conditions first,last"
            )
        if not has_fl_condition:
            raise ValueError(
                "MiniMax-H3 teacher matching requires FL2VA-style latent caches with first/last conditions;"
                " re-run minimax_h3_native_cache_latents.py --task fl2va"
            )
...
class MiniMaxH3NetworkTrainer(NetworkTrainer):
    ...
    def sample_timesteps(
        ...
        """Draw H3's shared, unshifted base time without requiring a newer trainer base.

        The native H3 schedule permits uniform base-time sampling only. Keeping this small
        compatibility seam local avoids replacing this fork's extended shared sampler while
        retaining min/max clipping, cached deterministic draws, and timestep buckets.
        """
        del latents
        ...
        if timesteps is not None:
            base = torch.as_tensor(timesteps, device=device, dtype=torch.float32).reshape(-1)
            if base.numel() != batch_size:
                raise ValueError(
                    f"MiniMax-H3 expected {batch_size} cached timestep value(s), got {base.numel()}"
                )
        elif args.preserve_distribution_shape and self.num_timestep_buckets is not None:
            base = torch.tensor(
        ...
        return base.mul(upper - lower).add(lower)
    ...
    def handle_model_specific_args(self, args: argparse.Namespace):
        ...
        if args.video_only and args.audio_only:
            raise ValueError("MiniMax-H3 --video_only and --audio_only are mutually exclusive")
        if args.audio_only and args.audio_loss_weight <= 0:
            raise ValueError("MiniMax-H3 --audio_only requires --audio_loss_weight greater than zero")
        ...
        if getattr(args, "h3_teacher_matching", False):
            if getattr(args, "h3_training_assistant_enabled", False):
                raise ValueError("MiniMax-H3 teacher matching and the Ostris training assistant are separate alternatives; enable only one")
            if args.task != "t2va":
        ...
        if guidance_scale > 0.0:
            if not args.h3_guidance_loss_uncond_cache:
                raise ValueError(
                    ...
                    " (write one with minimax_h3_native_cache_text_encoder_outputs.py --uncond_output)"
                )
    ...
    def sample_image_inference(
        ...
        if self._training_assistant is not None:
            # Match AI Toolkit: the helper shapes training but is absent from
            # previews and is never saved into the user's LoRA.
        ...
        try:
            ...
            if wandb_tracker is not None:
                try:
                    ...
                    media = wandb.Image(str(output_path)) if frame_count == 1 else wandb.Video(str(output_path), fps=24)
                    wandb_tracker.log({f"sample_{prompt_index}": media}, step=steps)
            return output_path
        finally:
            if self._training_assistant is not None:
    ...
    def extra_metadata(self, args: argparse.Namespace) -> dict:
        metadata = {
            ...
            "ss_minimax_h3_audio_only": args.audio_only,
    ...
    def process_batch(
        ...
        if args.audio_only and runtime.audio_present.item() != 1.0:
            raise ValueError(
                "MiniMax-H3 audio-only training requires real audio for every item; this cache item has no audio"
            )
...
def minimax_h3_setup_parser(parser: argparse.ArgumentParser) -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="uncond probe embedding for the guidance loss, written by minimax_h3_native_cache_text_encoder_outputs.py --uncond_output",
    )
    parser.add_argument(
        "--h3_teacher_matching",
        action="store_true",
