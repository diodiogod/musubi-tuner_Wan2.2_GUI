...
from musubi_tuner.minimax_h3_native.media import H3Task
...
class H3TimeOverrides:
    """Explicit RoPE times for a one-frame layout, relative to the target-block cursor."""

    condition_times: tuple[float, ...]
    target_time: float

    def __post_init__(self) -> None:
        values = (*((f"condition time {i}", value) for i, value in enumerate(self.condition_times)), ("target time", self.target_time))
...
def build_h3_layout(
    *,
    ...
    elif condition_roles is not None:
...
def build_position_grid(layout: H3PackedLayout, *, device: torch.device | str | None = None) -> torch.Tensor:
    ...
    if layout.task == "fl2va":
        ...
        if layout.time_overrides is not None:
            condition_times = [cursor + value for value in layout.time_overrides.condition_times]
        else:
